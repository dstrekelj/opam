      ...
      let f =
        OpamFile.Comp.create_preinstalled name version
          ...
          [ "CAML_LD_LIBRARY_PATH", Eq,
            Printf.sprintf "%%{lib}%%%sstublibs" Filename.dir_sep ^ String.make 1 (OpamStd.Sys.path_sep ()) ^
...
let string_of_env_update t shell updates =
  ...
  let cmd  p (k,v,comment) = Printf.sprintf "%sset %s=%s\n" p k v in
...
let dot_profile_needs_update t dot_profile shell =
  if OpamFilename.exists dot_profile || shell = `cmd then (
    let body =
      if shell = `cmd then
        ""
      else
    ...
    let pattern2 = escape @@ OpamFilename.to_string (OpamPath.init t.root // "init") in
    let pattern3 =
      escape @@ OpamStd.String.remove_prefix
...
let update_dot_profile t dot_profile shell =
  ...
  match dot_profile_needs_update t dot_profile shell with
    ...
    let body =
      Printf.sprintf
        "%s\n\n\
         ...
         %s"
        (OpamStd.String.strip body) (rem shell) (source t ~shell init_file) in
    OpamFilename.write dot_profile body; true

let update_setup_aux t user global =
  let updated =
...
let update_setup t user global = ignore (update_setup_aux t user global)
...
let print_env_warning_at_init t user =
  if up_to_date_env t then ()
    ...
    let (env_needed, profile_index, ocamlinit_index) =
      if user.shell <> `cmd && user.shell <> `clink then
        (true, "2.", "3.")
      else
        (false, "1.", "2.") in
    ...
    let env_line = Printf.sprintf
      "%s To configure OPAM in the current shell session, you need to run:\n\
       \n\
      \      %s\n"
      (OpamConsole.colorise `yellow "1.") (eval_string t)
    in
    OpamConsole.msg
      "\n%s\n\n\
       %s%s%s%s\n\n"
...
let check_and_print_env_warning t =
  if is_switch_globally_set t && not (up_to_date_env t) then
    if OpamStd.Sys.(os () = Win32) then
      set_cmd_env (get_opam_env ~force_path:false t)
    else
      OpamConsole.formatted_msg
...
let update_setup_interactive t shell dot_profile =
  let update dot_profile =
    ...
    OpamConsole.msg "\n";
    if update_setup_aux t user (Some {complete=true; switch_eval=true}) && shell = `clink then
      OpamConsole.msg
        "\n\
  ...
  let str_shell = OpamTypesBase.string_of_shell shell in
    ...
    if will_autorun then
      ...
      if has_clink then
        ...
        Printf.sprintf
          ...
          \    on startup. Specifically, it checks for and appends the following line:\n"
          (OpamConsole.colorise `cyan @@ OpamFilename.prettify dot_profile)
          (OpamConsole.colorise `bold @@ str_shell) in
