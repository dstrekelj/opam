(**************************************************************************)
(*                                                                        *)
(*    Copyright 2012-2013 OCamlPro                                        *)
(*    Copyright 2012 INRIA                                                *)
(*                                                                        *)
(*  All rights reserved.This file is distributed under the terms of the   *)
(*  GNU Lesser General Public License version 3.0 with linking            *)
(*  exception.                                                            *)
(*                                                                        *)
(*  OPAM is distributed in the hope that it will be useful, but WITHOUT   *)
(*  ANY WARRANTY; without even the implied warranty of MERCHANTABILITY    *)
(*  or FITNESS FOR A PARTICULAR PURPOSE.See the GNU General Public        *)
(*  License for more details.                                             *)
(*                                                                        *)
(**************************************************************************)

type profile = URI | Native | Command | Display

(* @@DRA No check that a basename isn't constructed with a directory separator? *)
module Base = OpamMisc.Base

let log fmt = OpamGlobals.log "FILENAME" fmt
let slog = OpamGlobals.slog

let forward_to_back = String.map (function '/' -> '\\' | c -> c)
let back_to_forward = String.map (function '\\' -> '/' | c -> c)

module Dir = struct

  include OpamMisc.Base

  let of_string dirname =
    let dirname =
      if (String.length dirname >= 1 && dirname.[0] = '~') then
        let home = OpamMisc.getenv "HOME" in
        match dirname with
        | "~" -> home
        | _   ->
          let prefix = Filename.concat "~" "" in
          let suffix = OpamMisc.remove_prefix ~prefix dirname in
          Filename.concat home suffix
      else if Filename.is_relative dirname then
        OpamSystem.real_path dirname
      else
        dirname
    in
      (*
       * Ensure on Windows that names consistently use backslashes
       *)
      if OpamMisc.os () = OpamMisc.Win32 then
        forward_to_back dirname
      else
        dirname

  let to_string =
    if Filename.dir_sep = "\\" then
      fun profile dirname ->
        match profile with
        | URI | Command ->
            back_to_forward dirname
        | _ ->
            dirname
    else
      fun _ dirname -> dirname

end

let raw_dir =
  if OpamMisc.os () = OpamMisc.Win32 then
    forward_to_back
  else
    fun s -> s

let with_tmp_dir fn =
  OpamSystem.with_tmp_dir (fun dir -> fn (Dir.of_string dir))

let rmdir dirname =
  log "rmdir %a" (slog (Dir.to_string Command)) dirname;
  OpamSystem.remove (Dir.to_string Command dirname)

let cwd () =
  Dir.of_string (Unix.getcwd ())

let mkdir dirname =
  OpamSystem.mkdir (Dir.to_string Native dirname)

let cleandir dirname =
  log "cleandir %a" (slog (Dir.to_string Native)) dirname;
  OpamSystem.remove (Dir.to_string Native dirname);
  mkdir dirname

let rec_dirs d =
  let fs = OpamSystem.rec_dirs (Dir.to_string Native d) in
  List.rev (List.rev_map Dir.of_string fs)

let dirs d =
  let fs = OpamSystem.dirs (Dir.to_string Native d) in
  List.rev (List.rev_map Dir.of_string fs)

let dir_is_empty d =
  OpamSystem.dir_is_empty (Dir.to_string Native d)

let in_dir dirname fn =
  if Sys.file_exists dirname then
    OpamSystem.in_dir dirname fn
  else
    OpamSystem.internal_error "Cannot CD to %s: the directory does not exist!" dirname

let exec dirname ?env ?name ?metadata ?keep_going cmds =
  let env = match env with
    | None   -> None
    | Some l -> Some (Array.of_list (List.rev_map (fun (k,v) -> k^"="^v) l)) in
  in_dir dirname
    (fun () -> OpamSystem.commands ?env ?name ?metadata ?keep_going cmds)

let move_dir ~src ~dst =
  OpamSystem.command [ "mv"; Dir.to_string Command src; Dir.to_string Command dst ]

let exists_dir dirname =
  let f = Dir.to_string Native dirname in
  Sys.file_exists f && Sys.is_directory f

let copy_dir ~src ~dst =
  if exists_dir dst then
    OpamSystem.internal_error
      "Cannot create %s as the directory already exists." (Dir.to_string Native dst);
  OpamSystem.command [ "cp"; "-PR"; Dir.to_string Command src; Dir.to_string Command dst ]

let link_dir ~src ~dst =
  if exists_dir dst then
    OpamSystem.internal_error "Cannot link: %s already exists." (Dir.to_string Native dst)
  else (
    mkdir (Filename.dirname dst);
    OpamSystem.link (Dir.to_string Native src) (Dir.to_string Native dst)
  )

let basename_dir dirname =
  Base.of_string (Filename.basename (Dir.to_string Native dirname))

let dirname_dir dirname =
  Dir.to_string Native (Filename.dirname (Dir.of_string dirname))

let to_list_dir dir =
  let base d = Dir.of_string (Filename.basename (Dir.to_string Native d)) in
  let rec aux acc dir =
    let d = dirname_dir dir in
    if d <> dir then aux (base dir :: acc) d
    else base dir :: acc in
  aux [] dir

let (/) d1 s2 =
  let s1 = Dir.to_string Native d1 in
  raw_dir (Filename.concat s1 s2)

type t = {
  dirname:  Dir.t;
  basename: Base.t;
}

let create dirname basename =
  let b1 = Filename.dirname (Base.to_string basename) in
  let b2 = Base.of_string (Filename.basename (Base.to_string basename)) in
  if basename = b2 then
    { dirname; basename }
  else
    { dirname = dirname / b1; basename = b2 }

let of_basename basename =
  let dirname = Dir.of_string "." in
  { dirname; basename }

let raw str =
  let dirname = raw_dir (Filename.dirname str) in
  let basename = Base.of_string (Filename.basename str) in
  create dirname basename

let to_string =
  if Filename.dir_sep = "\\" then
    fun profile t ->
      let result = Filename.concat (Dir.to_string Native t.dirname) (Base.to_string t.basename) in
      match profile with
      | URI | Command ->
          back_to_forward result
      | _ ->
          result
  else
    fun _ t ->
      Filename.concat (Dir.to_string Native t.dirname) (Base.to_string t.basename)

let uri_to_string = to_string

let digest t =
  Digest.to_hex (Digest.file (to_string Native t))

let touch t =
  OpamSystem.write (to_string Native t) ""

let chmod t p =
  Unix.chmod (to_string Native t) p

let of_string s =
  let dirname = Filename.dirname s in
  let basename = Filename.basename s in
  {
    dirname   = Dir.of_string dirname;
    basename  = Base.of_string basename;
  }

let dirname t = t.dirname

let basename t = t.basename

let read filename =
  OpamSystem.read (to_string Native filename)

let open_in filename =
  open_in (to_string Native filename)

let open_out filename =
  open_out (to_string Native filename)

let write filename raw =
  OpamSystem.write (to_string Native filename) raw

let writeRegistry filename =
  let (root, filename) =
    let i = String.index filename.dirname '\\' in
    (String.sub filename.dirname 0 i, {filename with dirname = String.sub filename.dirname (i + 1) (String.length filename.dirname - i - 1)}) in
  OpamMisc.writeRegistry (OpamMisc.regroot_of_string root) filename.dirname filename.basename

let remove filename =
  OpamSystem.remove_file (to_string Native filename)

let exists filename =
  let f = to_string Native filename in
  Sys.file_exists f && not (Sys.is_directory f)

let with_contents fn filename =
  fn (read filename)

let check_suffix filename s =
  Filename.check_suffix (to_string Native filename) s

let add_extension filename suffix =
  of_string ((to_string Native filename) ^ "." ^ suffix)

let chop_extension filename =
  of_string (Filename.chop_extension (to_string Native filename))

let rec_files d =
  let fs = OpamSystem.rec_files (Dir.to_string Native d) in
  List.rev_map of_string fs

let files d =
  let fs = OpamSystem.files (Dir.to_string Native d) in
  List.rev_map of_string fs

let copy ~src ~dst =
  if src <> dst then OpamSystem.copy (to_string Command src) (to_string Command dst)

let install ?exec ~src ~dst () =
  if src <> dst then OpamSystem.install ?exec (to_string Command src) (to_string Command dst)

let move ~src ~dst =
  if src <> dst then OpamSystem.command [ "mv"; to_string Command src; to_string Command dst ]

let link ~src ~dst =
  if src <> dst then OpamSystem.link (to_string Native src) (to_string Native dst)

let readlink src =
  if exists src then
    try of_string (Unix.readlink (to_string Native src))
    with Unix.Unix_error _ -> src
  else
    OpamSystem.internal_error "%s does not exist." (to_string Native src)

let is_symlink src =
  try
    let s = Unix.lstat (to_string Native src) in
    s.Unix.st_kind = Unix.S_LNK
  with Unix.Unix_error _ ->
    OpamSystem.internal_error "%s does not exist." (to_string Native src)

let is_exec file =
  try OpamSystem.is_exec (to_string Native file)
  with Unix.Unix_error _ ->
    OpamSystem.internal_error "%s does not exist." (to_string Native file)

let starts_with dirname filename =
  OpamMisc.starts_with ~prefix:(Dir.to_string Native dirname) (to_string Native filename)

let remove_prefix prefix filename =
  let prefix =
    let str = Dir.to_string Native prefix in
    if str = "" then "" else Filename.concat str "" in
  let filename = to_string Native filename in
  OpamMisc.remove_prefix ~prefix filename

let process_in ?root fn src dst =
  let basename = match root with
    | None   -> basename src
    | Some r ->
      if starts_with r src then remove_prefix r src
      else OpamSystem.internal_error "%s is not a prefix of %s"
          (Dir.to_string Native r) (to_string Native src) in
  let dst = Filename.concat (Dir.to_string Native dst) basename in
  fn ~src ~dst:(of_string dst)

let copy_in ?root = process_in ?root copy

let link_in = process_in link

let extract filename dirname =
  OpamSystem.extract (to_string Command filename) (Dir.to_string Command dirname)

let extract_in filename dirname =
  OpamSystem.extract_in (to_string Command filename) (Dir.to_string Command dirname)

type generic_file =
  | D of Dir.t
  | F of t

let extract_generic_file filename dirname =
  match filename with
  | F f ->
    log "extracting %a to %a"
      (slog (to_string Native)) f
      (slog (Dir.to_string Native)) dirname;
    extract f dirname
  | D d ->
    if d <> dirname then (
      log "copying %a to %a"
        (slog (Dir.to_string Native)) d
        (slog (Dir.to_string Native)) dirname;
      copy_dir ~src:d ~dst:dirname
    )

let ends_with suffix filename =
  OpamMisc.ends_with ~suffix (to_string Native filename)

let remove_suffix suffix filename =
  let suffix = Base.to_string suffix in
  let filename = to_string Native filename in
  OpamMisc.remove_suffix ~suffix filename

let download ~overwrite ?compress filename dirname =
  mkdir dirname;
  let dst = to_string Command (create dirname (basename filename)) in
  let file = OpamSystem.download ~overwrite ?compress
      ~filename:(to_string URI filename) ~dst in
  of_string file

let download_as ~overwrite ?(compress=false) filename dest =
  mkdir (dirname dest);
  let file = OpamSystem.download ~overwrite ~compress
      ~filename:(to_string URI filename) ~dst:(to_string Command dest) in
  assert (file = to_string Command dest);
  ()

let download_iter ~overwrite filenames dirname =
  let rec aux = function
    | []   ->
      let filenames = List.map (to_string URI) filenames in
      OpamSystem.internal_error "Cannot download %s." (OpamMisc.pretty_list filenames)
    | h::t ->
      try download ~overwrite h dirname
      with e -> OpamMisc.fatal e; aux t in
  aux filenames

let patch filename dirname =
  in_dir dirname (fun () -> OpamSystem.patch (to_string Command filename))

let with_flock ?read file f x =
  let lock = OpamSystem.flock ?read (to_string Native file) in
  try
    let r = f x in
    OpamSystem.funlock lock;
    r
  with e ->
    OpamMisc.register_backtrace e;
    OpamSystem.funlock lock;
    raise e

let checksum f =
  if exists f then
    [digest f]
  else
    []

let checksum_dir d =
  if exists_dir d then
    List.map digest (rec_files d)
  else
    []

let prettify_dir d =
  OpamMisc.prettify_path (Dir.to_string Native d)

let prettify s =
  OpamMisc.prettify_path (to_string Native s)

let to_json x = `String (to_string Native x)

module O = struct
  type tmp = t
  type t = tmp
  let compare = compare
  let to_string = to_string Native
  let to_json = to_json
end

module Map = OpamMisc.Map.Make(O)
module Set = OpamMisc.Set.Make(O)

let copy_files ~src ~dst =
  let files = rec_files src in
  List.iter (fun file ->
      if not !OpamGlobals.do_not_copy_files then
        let base = remove_prefix src file in
        let dst_file = create dst (Base.of_string base) in
        OpamGlobals.msg "Copying %s %s %s/\n"
          (prettify file)
          (if exists dst_file then "over" else "to")
          (prettify_dir dst);
        copy ~src:file ~dst:dst_file
    ) files

module OP = struct

  let (/) = (/)

  let (//) d1 s2 =
    let d = Filename.dirname s2 in
    let b = Filename.basename s2 in
    if d <> "." then
      create (d1 / d) (Base.of_string b)
    else
      create d1 (Base.of_string s2)

end

module Attribute = struct

  type t = {
    base: Base.t;
    md5 : string;
    perm: int option;
  }

  let base t = t.base

  let md5 t = t.md5

  let perm t = t.perm

  let create base md5 perm =
    { base; md5; perm=Some perm }

  let to_string_list t =
    let perm = match t.perm with
      | None   -> []
      | Some p -> [Printf.sprintf "0o%o" p] in
    Base.to_string t.base :: t.md5 :: perm

  let of_string_list = function
    | [base; md5]      -> { base=Base.of_string base; md5; perm=None }
    | [base;md5; perm] -> { base=Base.of_string base; md5;
                            perm=Some (int_of_string perm) }
    | k                -> OpamSystem.internal_error
                            "remote_file: '%s' is not a valid line."
                            (String.concat " " k)

  let to_string t = String.concat " " (to_string_list t)
  let of_string s = of_string_list (OpamMisc.split s ' ')

  let to_json x =
    `O ([ ("base" , Base.to_json x.base);
          ("md5"  , `String x.md5)]
        @ match x. perm with
          | None   -> []
          | Some p -> ["perm", `String (string_of_int p)])

  module O = struct
    type tmp = t
    type t = tmp
    let to_string = to_string
    let compare = compare
    let to_json = to_json
  end

  module Set = OpamMisc.Set.Make(O)

  module Map = OpamMisc.Map.Make(O)

end

let to_attribute root file =
  let basename = Base.of_string (remove_prefix root file) in
  let perm =
    let s = Unix.stat (to_string Native file) in
    s.Unix.st_perm in
  let digest = digest file in
  Attribute.create basename digest perm
