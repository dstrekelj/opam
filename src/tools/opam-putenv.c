 ...
 * rather than having to deal with .o(obj) files for different architectures.
 */
 ...
 * and allows OPAM to manipulate a parent whose architecture differs from its own.
 * When the architecture matches, OPAM injects the code itself, but injecting
 * from a 64-bit process to a 32-bit parent is quite hard (and potentially
 * unstable) and injecting from a 32-bit process to a 64-bit parent is phenomenally
 * hard!
 */
