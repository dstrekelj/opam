...
-- @@DRA FIXME Various commands taking too long to run for readline completion
-- @@DRA It may be a weird artefact of my build virtual machine, however...
-- Two possible fixes:
...
--   1. There is a delay on launching any command prompt before completion will be available
--      (while the coroutines execute)
--   2. New packages added during a session won't be detected (although perhaps 

local function cmd_to_lines(cmd)
  local result = {}
...
if installed then
  coroutine.resume(opam_packages, "opam list --safe -a -s 2>nul")
  coroutine.resume(opam_switches, "opam switch list --safe -s -a 2>nul")
end
